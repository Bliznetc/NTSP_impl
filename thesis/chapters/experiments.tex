...
\paragraph{The diamond-chain panel inverts the picture.} The
diamond-chain sub-panel of Figure~\ref{fig:runtime} shows the same axes as
the other four. Yen's curve climbs steeply -- from microseconds
...
Two quite different things can make an NSP absent, and it is worth keeping
them apart.

The first is an artefact of sparse random generation: a graph in which $t$ is
simply \emph{unreachable} from $s$ has no shortest path, so the question of a
next-to-shortest one does not arise. Raw draws produced such graphs often at
small $|V|$ -- $13/20$ Erd\H{o}s--R\'enyi instances at $|V|=6$ -- and in the
layered family the cause was structural rather than statistical: with
$\text{width}=2$ and $p_f=0.40$, a pair of consecutive layers receives no
forward edge at all with probability
$(1-p_f)^{\text{width}^2}=0.6^{4}\approx 13\%$, severing the graph. Merely
forcing one forward edge per transition does not repair this, since that edge
may itself start at an already-unreachable vertex; the generator therefore
tracks reachability as it builds and repairs out of a vertex known to be
reachable. The benchmark additionally redraws any instance in which $t$ is
unreachable. Between them, all $1200$ instances reported here pose a
well-formed question: $t$ is reachable from $s$, so $d(s,t)$ exists.

That is emphatically \emph{not} the same as guaranteeing an answer, which is
the second effect and the interesting one. Of the $1200$ instances, $1131$
have an NSP and $69$ do not, every one of the latter at $|V|\le 20$. Those are
graphs with exactly one simple $s\to t$ path -- which is then the shortest
path by default, leaving nothing to be next-to-shortest -- or, in a handful of
cases, several paths whose costs all tie at the minimum, so that none of them
is \emph{not}-shortest. Both are perfectly legitimate NSP instances whose
correct answer is ``none'', and all three algorithms are required to say so;
we keep them deliberately, as they are the benchmark's only coverage of that
code path. The effect disappears once the graphs are large enough to admit
several distinct-cost routes: every family is at $100\%$ from $|V|=25$ upward.
...
The diamond-chain family in Section~\ref{sec:methodology} is repeated here with a
...
\begin{table}[htbp]
  ...
  \caption{Diamond-chain adversarial benchmark. Yen-NSP must enumerate
    ...
    figure is the median of $11$ repetitions.}
  \label{tab:adv}
\end{table}
...
\begin{itemize}
  \item Our 2-VDP-in-DAG subroutine is the $O(|V|^{2}+|V||E|)$
    Fortune--Hopcroft--Wyllie algorithm (Section~\ref{sec:twovdp}), not
    Tholey's linear-time algorithm~\cite{Tholey2012}, so the
    implementation does not attain the paper's asymptotic bound. We have
    not measured how much of the observed runtime this accounts for.
...
\begin{itemize}
  ...
  \item All experiments stop at a sweep target of $|V|=70$, which given the
    gap noted above means actual sizes up to $81$ (grid) and $74$ (layered).
    They use integer weights drawn
