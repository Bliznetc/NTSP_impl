...
\begin{enumerate}
  \item The algorithm is implementable from the paper's pseudocode
    without Tholey's linear-time 2-VDP routine. The
    Fortune--Hopcroft--Wyllie algorithm for two paths in a DAG, which
    runs in $O(|V|^{2}+|V||E|)$ time, is enough for a working
    implementation, at the cost of a factor of $|V|$ per 2-VDP call.
  \item On the four benign random graph families we measured
    (Erd\H{o}s--R\'enyi, random DAG, layered, grid) at $|V|\le 70$,
  ...
  \item On a deliberate adversarial family of \emph{diamond-chain}
    ...
    At $k=15$ ($|V|=46$, $|E|=61$) Yen takes $56$~seconds while CWZ
    finishes in $22$~microseconds -- a ${\sim}2.6$-million-fold ratio
  ...
  \item Our implementation is a faithful transcription of the paper's
    ...
    \emph{zero} mismatches across $34{,}000$ random instances
    ($|V|\le 14$) and matches Yen on every benign benchmark instance up
    to $|V|=70$. The $6$-tuple enumeration alone
...
\paragraph{Engineering improvements.} Replacing our quadratic 2-VDP by
Tholey's linear-time algorithm~\cite{Tholey2012} would remove a factor
of $|V|$ from every 2-VDP call. Eliminating the
...
\paragraph{Larger graphs.} Our experiments stop at $|V|\approx 70$. The
empirical exponent depends strongly on the family, ranging from
$\approx 1.3$ on diamond chains to $\approx 4.3$ on dense
Erd\H{o}s--R\'enyi graphs, in every case well below the worst-case
$|V|^{4}|E|^{3}$ bound; characterising this gap on still larger or
differently-structured inputs would be valuable.
...
queries across nearby tuples, as well as replacing our quadratic 2-VDP
