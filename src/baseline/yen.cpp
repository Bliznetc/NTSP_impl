...
// Dijkstra avoiding blocked vertices and edges.
...
// Edge list src->tgt from a parent array.
...
NspResult yen_nsp(const Graph& g, VertexId s, VertexId t, int k_max) {
    ...
    // The recovered path must match dist[t].
    assert(path_cost(g, A.front()) == r.shortest_cost);

    // Candidate paths by cost, deduplicated by edge sequence.
    struct Candidate {
        Weight cost;
    ...
    for (int k = 1; k < k_max; ++k) {
        ...
        // Root path vertices, blocked in the spur search.
        std::vector<VertexId> root_vertices;
        root_vertices.push_back(s);

        for (std::size_t i = 0; i < prev.size(); ++i) {
            VertexId spur_node = g.edge(prev[i]).src;
            // Block the i-th edge of every accepted path sharing this root.
    ...
    // k_max reached: possible false negative.
    return r;
}
