...
ReductionResult reduce_to_straight(const Graph& g, VertexId s, VertexId t) {
    // Fold off-SP vertices one at a time: in-edge (x,u) + out-edge (u,y) becomes
    // a shortcut (x,y) carrying its original edges. Repeated folding handles
    // chains of off-SP vertices.
    ...
    if (dst >= kInfWeight) {
        // t unreachable: keep only s and t.
        VertexId new_n = 0;
    ...
    struct WorkEdge {
        ...
        VertexId dst;
        Weight w;
        std::vector<EdgeId> provenance;  // original edges, in order
    };
    ...
    // Fold each off-SP vertex; the order does not affect the result.
    for (VertexId u = 0; u < n; ++u) {
        if (on_sp[u]) continue;

        // Split edges into in/out of u and the rest.
        ...
        // Shortcut every (in, out) pair, skipping would-be self-loops.
        ...
        // Keep the K cheapest distinct weights per (src, dst).
        {
    ...
    // Build the reduced graph on the on-SP vertices.
    VertexId new_n = 0;
    for (VertexId v = 0; v < n; ++v) {
...
// Shortest s->v path from a forward Dijkstra parent array.
...
// Shortest v->t path from a reverse Dijkstra parent array.
...
ReductionResult reduce_to_layered(const Graph& g, VertexId s, VertexId t) {
    ...
    // Layer index = rank among distinct distances.
    ...
    for (EdgeId i = 0; i < g.num_edges(); ++i) {
        ...
        if (!is_forward) {
            // Back-edge: drop it if d(u) <= d(v), recording its candidate path;
            // keep it if strictly backward.
        ...
        // Subdivide a layer-skipping forward edge; only the first sub-edge maps to
        // the original edge, so the chain expands to it once.
        VertexId prev = e.src;
...
ReductionResult reduce_full(const Graph& g, VertexId s, VertexId t) {
    ...
    // Translate path_expand through r1 to original edges.
    ...
    // r1 candidates are in original edges; translate r2's through r1.
